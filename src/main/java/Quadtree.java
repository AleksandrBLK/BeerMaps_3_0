...
public class Quadtree {
    ...
    protected static class Node {
        ...
        private double lowrightY;
        private double lowrightX;
        private int index;
        private double XDisPerPixel;
        private Node[] children;
        private int childIndex;
        protected String dir;
        private int level;

        protected Node(int n, Double ulY, Double ulX, Double lrY, Double lrX, int depth) {
            dir = "d" + (8 - depth)
                    + "_x" + ((int) (Math.pow(2, 8 - depth) * ((ulX + lrX) / 2 - MapServer.ROOT_ULLON) / (MapServer.ROOT_LRLON - MapServer.ROOT_ULLON)))
                    + "_y" + ((int) ((Math.pow(2, 8 - depth) * ((ulY + lrY) / 2 - MapServer.ROOT_ULLAT) / (MapServer.ROOT_LRLAT - MapServer.ROOT_ULLAT))));
            upleftY = ulY;
            upleftX = ulX;
            lowrightY = lrY;
            lowrightX = lrX;
            index = n;
            XDisPerPixel = (lowrightX - upleftX) / MapServer.TILE_SIZE;
            childIndex = 0;
            children = new Node[4];
            level = depth;
        }
    ...
    protected Quadtree(Node r, int level) {
    ...
    private Node autoAddChildren(Node n, int level) {
        if (level == 1) {
            ...
            Node child = new Node(index, upleftY, upleftX, lowrightY, lowrightX, level - 1);
            ...
            lowrightX += xInterval;
            upleftX += xInterval;
            child = new Node(index, upleftY, upleftX, lowrightY, lowrightX, level - 1);
            n.addChild(autoAddChildren(child, level - 1));
    ...
    protected void getRaster(Map<String, Double> params, Node n, Double queryxDisPerPixel,
            ...
            if (n.level != 1 && n.XDisPerPixel > queryxDisPerPixel) {
                ...
                if (nodesLst.size() == 0) {
                    ...
                    while (index < size && !existRow) {
                        ...
                        if (firstNode.upleftY == (n.upleftY)) {
